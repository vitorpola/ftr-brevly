...
export const createLinkInput = z.object({
  shortUrl: z.string()
    ...
    .regex(/^[a-z0-9-_]+$/, {
      message: "A URL encurtada deve conter apenas letras minúsculas, números, hífen ou underline"
    }),
...
export async function createLink(input: CreateLinkInput): Promise<Either<Error, string>> {
  try {
    ...
    return makeLeft(error instanceof Error ? error : new Error('Um erro desconhecido ocorreu.'))
  }
}